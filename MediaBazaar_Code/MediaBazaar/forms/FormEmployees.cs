...
namespace MediaBazaar.forms
{
    public partial class FormEmployees : Form
    {
        ...
        public FormEmployees(EmployeeAdministration empAdmin, Manager m, ProductAdministration prodAdmin)
        {
            InitializeComponent();
            ...
            this.managers = this.empAdmin.GetAllManagers();
        ...
        {
            if (String.IsNullOrEmpty(this.tbIdEmployee.Text))
            {
                MessageBox.Show("Enter the id of the employee you want to remove!");
            }
            else
            {
                int removeId = Convert.ToInt32(this.tbIdEmployee.Text);
                Employee emp = null;
        ...
        private void btnUpdateEmployee_Click_1(object sender, EventArgs e)
        {
            if (String.IsNullOrEmpty(this.tbIdEmployee.Text))
            {
                MessageBox.Show("Enter the id of the employee you want to update!");
            }
            else
            {
                int updateId = Convert.ToInt32(this.tbIdEmployee.Text);
                Employee emp = null;
                foreach (Employee em in DatabaseHelper.GetAllEmployees())
                {
                    if (em.Id == updateId)
        ...
        private void btnAssign_Click(object sender, EventArgs e)
        {
            if (String.IsNullOrEmpty(this.tbIdEmployee.Text))
            {
                MessageBox.Show("Enter the id of the employee you want to assign!");
            }
            else
            {
                int assignId = Convert.ToInt32(this.tbIdEmployee.Text);
                Employee emp = null;
                foreach (Employee em in DatabaseHelper.GetAllEmployees())
                {
                    if (em.Id == assignId)
